package io.jon.rpc.fusing.base;
...
public abstract class AbstractFusingInvoker implements FusingInvoker {
    ...
    public void markSuccess() {
        if (fusingStatus.get() == RpcConstants.FUSING_STATUS_HALF_OPEN){
            // 成功将等待状态由成功更新为初始化，说明真实服务已经恢复，则熔断状态修改为关闭
            fusingWaitStatus.compareAndSet(RpcConstants.FUSING_WAIT_STATUS_WAITINF,
    ...
    public void markFailed() {
        ...
        if (fusingStatus.get() == RpcConstants.FUSING_STATUS_HALF_OPEN){
            // 成功将等待状态由失败更新为初始化，说明真实服务未恢复，则熔断状态修改为开启，进入下一个熔断时间窗口周期
            fusingWaitStatus.compareAndSet(RpcConstants.FUSING_WAIT_STATUS_WAITINF,
